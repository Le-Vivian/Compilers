...
ali_int errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern ali_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern ali_int line; /* source code line number - defined in scanner.c */
...
extern ali_void startParser(julius_void);
extern ali_int startScanner(ReaderPointer sc_buf);

static ali_void printParserError(ali_char* fmt, ...);
static ali_void displayParser(ReaderPointer ptrBuffer);
static ali_long getParserFileSize(ali_char* fname);
static ali_void callGarbageCollector(julius_void);
...
ali_int mainParser(ali_int argc, ali_char** argv) {

	FILE* fi;       /* input file handle */
	ali_int loadsize = 0; /*the size of the file loaded in the buffer */
...
ali_void printParserError(ali_char* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(ali_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
ali_long getParserFileSize(ali_char* fname) {
	FILE* input;
	ali_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
ali_void displayParser(ReaderPointer ptrBuffer) {
